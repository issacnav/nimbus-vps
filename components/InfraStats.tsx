...
function ChartBar({
  ...
  return (
    <motion.div
      ...
      className="flex-1 cursor-pointer rounded-t-[3px]"
      initial={{ scaleY: 0, originY: 1 }}
      animate={inView ? { scaleY: 1 } : { scaleY: 0 }}
      transition={{ duration: 0.55, ease: EASE, delay }}
      style={{
        height: `${heightPct}%`,
        background: hovered ? "#111" : "#555",
        originY: 1,
      }}
    />
  );
...
function WeeklyChart(): JSX.Element {
  ...
  return (
    <div className="overflow-hidden rounded-[14px] border border-[#ebebeb] bg-[#fafafa] shadow-[0_2px_8px_rgba(0,0,0,0.05)]">
      {/* Header */}
      <div className="flex items-center justify-between border-b border-[#f2f2f2] px-4 py-3">
        <div>
          <div className="text-[12px] font-bold text-[#111]">Weekly Resource Usage</div>
          <div className="text-[10px] text-[#aaa]">web-prod-01 · Last 7 days</div>
        </div>
        <div className="flex items-center gap-2">
          ...
          {(["cpu", "bw"] as const).map((key) => (
            ...
            >
              <span className="h-[6px] w-[6px] rounded-full bg-[#555]" />
              {key === "cpu" ? "CPU" : "Bandwidth"}
            </button>
          ))}
          <div className="rounded-[5px] border border-[#e8e8e8] bg-[#f5f5f5] px-2 py-[3px] text-[9px] font-semibold text-[#888]">
            7d ▾
          </div>
        </div>
      </div>
      ...
      <div className="px-4 py-4">
        {hoveredDay !== null && (
          ...
          >
            <span className="font-semibold text-[#111]">{days[hoveredDay]}</span>
            {(activeSeries === "both" || activeSeries === "cpu") && (
              <span className="text-[#555]">CPU: {cpuSeries[hoveredDay]}%</span>
            )}
            {(activeSeries === "both" || activeSeries === "bw") && (
              <span className="text-[#555]">BW: {bwSeries[hoveredDay]} GB</span>
            )}
          </motion.div>
        )}
        <div className={`flex h-[92px] items-end gap-2 ${hoveredDay !== null ? "" : "mt-[24px]"}`}>
          {days.map((day, i) => (
            ...
            >
              ...
              <div className={`text-[8.5px] transition-colors ${hoveredDay === i ? "text-[#111] font-semibold" : "text-[#aaa]"}`}>
                {day}
              </div>
            </div>
          ))}
      ...
      <div className="grid grid-cols-3 divide-x divide-[#f0f0f0] border-t border-[#f0f0f0]">
        {[
          ...
          <div key={m.label} className="px-4 py-3">
            <div className="text-[9px] font-semibold uppercase tracking-[0.06em] text-[#aaa]">
              {m.label}
            </div>
            <div className="mt-0.5 text-[14px] font-bold text-[#111]">
              {m.value}
            </div>
            <div className="text-[9px] text-[#aaa]">{m.sub}</div>
          </div>
        ))}
      </div>
    </div>
...
function FeatureHighlight(): JSX.Element {
  return (
    <motion.div
      ...
      className="overflow-hidden rounded-[22px] border border-[#e8e8e8] bg-white shadow-[0_2px_10px_rgba(0,0,0,0.04)]"
    >
      <div className="grid md:grid-cols-[1fr_1.45fr]">
        {/* Left — text */}
        <div className="flex flex-col justify-between p-8 md:p-10">
          <div>
            <div className="mb-5 inline-flex w-fit items-center rounded-full border border-[#e5e5e5] bg-[#f2f2f2] px-3 py-[5px] text-[11px] font-medium text-[#666]">
              Isolate Issues
            </div>
            <h3 className="mb-3 text-[clamp(20px,2.2vw,28px)] font-bold leading-[1.2] tracking-[-0.022em] text-[#111]">
              Real-Time Server Monitoring
            </h3>
            <p className="text-[14px] leading-[1.72] text-[#888]">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            className="mt-8 inline-flex w-fit items-center gap-2 rounded-full border border-[#e0e0e0] bg-[#f2f2f2] px-5 py-[11px] text-[13px] font-semibold text-[#111] transition-colors duration-150 hover:bg-[#e8e8e8]"
          >
            Explore Monitoring
        ...
        {/* Right — chart */}
        <div className="flex items-center border-t border-[#f0f0f0] bg-[#fafafa] p-6 md:border-l md:border-t-0 md:p-8">
  ...
  return (
    ...
    >
      <div className="mb-2.5 flex items-baseline gap-[2px]">
        {prefix && (
          <span className="font-google-sans text-[38px] font-extrabold leading-none tracking-[-0.03em] text-[#111]">
            {prefix}
          </span>
        )}
        <span
          ref={ref}
          className="font-google-sans text-[48px] font-extrabold leading-none tracking-[-0.03em] text-[#111]"
        >
          {count}
        </span>
        <span className="font-google-sans text-[26px] font-extrabold leading-none tracking-[-0.02em] text-[#666]">
          {suffix}
        </span>
      </div>
      <p className="text-[13.5px] leading-[1.5] text-[#555]">{label}</p>
    </motion.div>
  );
}
...
function StatsFrame(): JSX.Element {
  ...
  return (
    <div ref={ref} className="relative z-0 mt-20">
      ...
      <div className="absolute left-[-50vw] right-[-50vw] top-0 hidden h-px -translate-y-1/2 bg-[linear-gradient(to_right,transparent_0%,rgba(0,0,0,0.06)_20%,rgba(0,0,0,0.06)_80%,transparent_100%)] -z-10 md:block" />
      <div className="absolute bottom-0 left-[-50vw] right-[-50vw] hidden h-px translate-y-1/2 bg-[linear-gradient(to_right,transparent_0%,rgba(0,0,0,0.06)_20%,rgba(0,0,0,0.06)_80%,transparent_100%)] -z-10 md:block" />

      {/* Vertical fade lines (blueprint grid effect) */}
      <div className="absolute bottom-[-100px] left-0 top-[-100px] hidden w-px -translate-x-1/2 bg-[linear-gradient(to_bottom,transparent_0%,rgba(0,0,0,0.06)_20%,rgba(0,0,0,0.06)_80%,transparent_100%)] -z-10 md:block" />
      <div className="absolute bottom-[-100px] left-1/3 top-[-100px] hidden w-px -translate-x-1/2 bg-[linear-gradient(to_bottom,transparent_0%,rgba(0,0,0,0.06)_20%,rgba(0,0,0,0.06)_80%,transparent_100%)] -z-10 md:block" />
      <div className="absolute bottom-[-100px] left-2/3 top-[-100px] hidden w-px -translate-x-1/2 bg-[linear-gradient(to_bottom,transparent_0%,rgba(0,0,0,0.06)_20%,rgba(0,0,0,0.06)_80%,transparent_100%)] -z-10 md:block" />
      <div className="absolute bottom-[-100px] right-0 top-[-100px] hidden w-px translate-x-1/2 bg-[linear-gradient(to_bottom,transparent_0%,rgba(0,0,0,0.06)_20%,rgba(0,0,0,0.06)_80%,transparent_100%)] -z-10 md:block" />
      ...
      >
        <div className="grid grid-cols-1 divide-y divide-[rgba(0,0,0,0.06)] sm:grid-cols-3 sm:divide-x sm:divide-y-0">
    ...
    <section className="bg-[#f5f5f5] px-6 py-24 sm:px-10">
      <div className="mx-auto max-w-[1060px]">
        ...
        >
          ...
          <p className="mx-auto max-w-[500px] text-[16px] leading-[1.65] text-[#555]">
